...
def gen_blueprint(idea, source_url=None, source_path=None, variation=False, variation_opts=None):
    ...
    charter = (
        ...
        "homme » si c'est une fille). Capte surtout l'ÉMOTION et l'INTENTION "
        "RÉELLES : si les personnages sont FIERS, CONTENTS ou complices, ne les "
        "montre pas tristes ; reproduis le TON exact (drôle, sombre, sarcastique, "
        "fier…). Une émotion inversée casse toute la vidéo. Sois fidèle à la "
        "réalité de la vidéo.\n\n"
        "PRÉCISION MAXIMALE (les IA sont bêtes : si tu ne précises pas TOUT, "
        "elles font n'importe quoi — changent un perso, en ajoutent un autre). "
        "Chaque image_prompt doit décrire SANS AMBIGUÏTÉ : qui est présent (et "
        "PERSONNE d'autre), leur apparence exacte, leur ÉMOTION, l'action, le "
        "décor, le cadrage. Tout se joue sur l'image : si l'image est mauvaise, "
        "tout rate.\n\n"
        "CHRONOLOGIE (bug vu et interdit) : l'image d'un plan montre les "
        "personnages dans l'état EXACT où ils sont À CE MOMENT de l'histoire — "
        "vêtements, accessoires, état physique. JAMAIS un état futur (ex : le "
        "petit n'a PAS ENCORE l'uniforme de l'école pendant qu'on le peint ; "
        "l'uniforme n'apparaît QUE dans les plans après qu'il l'a reçu). Avant "
        "d'écrire chaque image_prompt, demande-toi : « qu'est-ce qui s'est "
        "passé jusqu'ici ? » et décris exactement CET état, y compris dans les "
        "images réutilisées.\n\n"
        ...
        "ÉCONOMIE D'IMAGES (LE PLUS IMPORTANT) : UNE image + UNE animation de "
        "jusqu'à 8 s enchaîne PLUSIEURS actions et répliques. Exemple : le petit "
        "cire la chaussure de l'homme -> l'homme dit « ... » -> sort de l'argent "
        "de sa poche -> le donne au petit -> le petit répond « ... » -> l'homme "
        "monte dans sa voiture -> son portefeuille tombe. TOUT ça = UN SEUL plan "
        "animé, UNE seule image. Ne génère une NOUVELLE image QUE si le décor OU "
        "les personnages changent vraiment (sinon nouvelle_image=false + "
        ...
        "guillemets (ex : the man says in French: « ... »).\n\n"
        "DEUX PROMPTS par plan, en anglais, dans l'ordre : image_prompt (si "
        "nouvelle image) ; puis animation_prompt qui décrit TOUTE la séquence du "
        ...
        "STRUCTURE OFFICIELLE VEO (doc Google, applique-la à la lettre) :\n"
        "- Formule d'un prompt : [Cinématographie] + [Sujet] + [Action] + "
        "[Contexte] + [Style & Ambiance].\n"
        "- TIMESTAMP PROMPTING officiel : découpe chaque animation_prompt en "
        "segments horodatés « [00:00-00:02] Medium shot, the rich banana-man in "
        "the white suit… [00:02-00:05] … [00:05-00:08] … ». C'est LE format que "
        "Veo suit le mieux — chaque action/réplique a son créneau.\n"
        "- Un clip Veo dure 4, 6 ou 8 s (jamais plus). Scène plus longue -> "
        "plusieurs plans de la MÊME image. Kling : ~10-15 s.\n\n"
        "AUDIO (règles officielles Veo) :\n"
        "- Les instructions AUDIO se placent TÔT dans le prompt (dans la 1re "
        "moitié) — à la fin, Veo les ignore.\n"
        "- Dialogue : X says, \"…\" (la réplique exacte entre guillemets). "
        "Une réplique = UNE respiration (courte, elle doit tenir dans le clip).\n"
        "- Effets : « SFX: thunder cracks in the distance ». Fond sonore : "
        "« Ambient noise: … ».\n"
        "- UN SEUL parleur par clip (seul = ~80% de réussite, plusieurs = "
        "<40%). Deux persos qui parlent -> DEUX clips séparés de la même image, "
        "ou séquence explicite : « The woman speaks first, then pauses. The "
        "man nods and replies. »\n\n"
        "RÈGLES D'OR DE L'ANIMATION :\n"
        "1. IDENTITÉ EXACTE DES PERSONNAGES (le plus important) :\n"
        "   a) Si le personnage N'EST PAS humain, n'utilise JAMAIS de mot "
        "humain — AUCUN : ni « man », « woman », « boy », « kid », ni « child », "
        "« son », « daughter », « baby », « parents », « family » — l'IA le "
        "transformerait en humain (c'est arrivé : « child » a changé la petite "
        "banane en petit garçon humain en fin de clip). Dis « the small young "
        "banana character », « the two adult banana characters ». Et décris son anatomie en "
        "AFFIRMATIF (les négations « no human face » ne marchent pas) : dis ce "
        "que le perso EST : « the character's entire head IS a smooth banana "
        "peel; its eyes ARE two big round white cartoon googly eyes sitting on "
        "the peel; its mouth IS a simple cartoon mouth drawn on the peel; its "
        "body IS a banana with thin cartoon limbs ».\n"
        "   b) Écris UNE description par personnage et RÉUTILISE-LA MOT POUR "
        "MOT partout (image_prompt ET animation_prompt de tous les plans). "
        "Toute reformulation = Veo réinterprète et change le personnage.\n"
        "   c) En animation (image->vidéo), ANCRE le prompt à l'image : "
        "commence par « Animate the exact characters and scene from the input "
        "image, keeping their appearance, faces, bodies and expressions 100% "
        "unchanged ». INTERDIT « the kid », « the man », « he/she » seuls.\n"
        "   d) STYLE VERROUILLÉ : identifie le STYLE DE RENDU exact de la "
        "vidéo source (ex : « stylized 3D cartoon render, Pixar-like ») et "
        "mets-le au DÉBUT de chaque image_prompt. Un style photoréaliste "
        "transforme les persos cartoon en hybrides humains.\n"
        "   e) IMAGE DE RÉFÉRENCE (l'arme la plus fiable) : dans 'materiel' et "
        "en 1re position de 'conseils_adaptation', dis au créateur de PRENDRE "
        "UNE CAPTURE D'ÉCRAN de la vidéo originale (le plan où on voit bien "
        "les personnages) et de la donner comme IMAGE DE RÉFÉRENCE à Nano "
        "Banana avec chaque prompt image — c'est LE moyen de garder les "
        "personnages identiques.\n"
        "2. PAROLE = bloc complet : QUI (description complète) + LA VOIX (deep "
        "raspy male voice, soft young female voice…) + le TON (smug, cruel, "
        "joyful…) + says, \"réplique exacte\" + l'action SIMULTANÉE : « WHILE "
        "saying \"Tiens\", the rich banana-man pulls a banknote from his "
        "pocket and hands it over ». Jamais la parole puis le geste plus tard.\n"
        "3. Non-parleurs FIGÉS : « the cucumber boy stays silent, mouth closed, "
        "listening. Only the banana-man's lips move ».\n"
        "4. CAMÉRA à chaque segment, vocabulaire officiel : medium shot, "
        "close-up, wide shot, low angle, dolly, tracking shot, slow pan, POV. "
        "Tu peux dévier un peu de la source (zoom sur le parleur) si c'est "
        "plus clair.\n"
        "5. ORDRE EXPLICITE : chaque geste est WHILE / AFTER / BEFORE une "
        "réplique. Jamais implicite.\n"
        "6. PAS DE SURCHARGE : max 3-4 actions par clip de 8 s, sinon Veo en "
        "saute -> découpe en un 2e plan de la même image.\n"
        "7. PHRASES AFFIRMATIVES (règle officielle) : Veo gère MAL les "
        "négations. Au lieu de « no woman », écris ce qui EST là : « only "
        "these two characters are in the scene from start to finish ». Ne "
        "mentionne QUE les personnages réellement présents.\n"
        "8. VERROUS finaux (affirmatifs) : « the characters keep exactly the "
        "same appearance for the whole clip; the camera holds one continuous "
        "shot; clean footage without any on-screen text or subtitles ».\n"
        "9. Une seule réplique à la fois (jamais 2 persos qui parlent "
        "ensemble), et RIEN QUE LE SCRIPT : ajoute « they speak ONLY the quoted "
        "lines; no other words are spoken » — Veo invente des répliques sinon.\n"
        "10. FRONTIÈRE DE SCÈNE STRICTE : un animation_prompt contient "
        "UNIQUEMENT ce qui se passe dans CE plan — jamais une réplique ou une "
        "action du plan suivant (elle appartient au clip suivant, avec sa "
        "propre image). Termine chaque prompt par l'état final EXACT : « the "
        "clip ends with [pose finale précise]; the characters then hold still; "
        "nothing else happens ». Sans ça, Veo continue l'histoire tout seul et "
        "transforme les personnages.\n\n"
        "ANALYSE STRICTE : avant d'écrire quoi que ce soit, regarde la vidéo "
        "comme un détective — chaque détail compte : l'ÉMOTION et l'INTENTION "
        "réelles de chaque personnage (des parents qui COMPLOTENT joyeusement "
        "ne sont PAS tristes), le ton des voix, les gestes exacts et leur "
        "timing, les angles de caméra. L'émotion apparaît dans l'image_prompt "
        "(expressions du visage) ET dans l'animation_prompt (ton de la voix, "
        "manière de bouger).\n\n"
